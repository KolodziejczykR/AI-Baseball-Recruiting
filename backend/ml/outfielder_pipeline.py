...
class OutfielderPredictionPipeline:
    def __init__(self, models_dir='models/'):
        """
        Initialize the outfielder prediction pipeline.
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.d1_model = joblib.load(os.path.join(self.models_dir, 'xgb_outfield_d1_or_not.pkl'))
            self.d1_scaler = joblib.load(os.path.join(self.models_dir, 'scalers_outfield.pkl'))
            self.d1_label_encoders = joblib.load(os.path.join(self.models_dir, 'label_encoders_outfield.pkl'))
            
            # Load Power 4 vs Non-Power 4 model and artifacts
            self.p4_model = joblib.load(os.path.join(self.models_dir, 'xgb_outfield_d1_p4_or_not.pkl'))
            self.p4_scaler = joblib.load(os.path.join(self.models_dir, 'scalers_outfield_d1_p4.pkl'))
            self.p4_label_encoders = joblib.load(os.path.join(self.models_dir, 'label_encoders_outfield_d1_p4.pkl'))
            
            print("Successfully loaded all outfielder models and preprocessing artifacts")
            
        except Exception as e:
            print(f"Error loading outfielder models: {e}")
            raise
    ...
    def _get_feature_columns(self) -> List[str]:
        ...
        exclude_columns = [
            'hard_hit_p', 'position_velo',  # Keep of_velo
        ...
        feature_columns = [
            ...
            'sweet_spot_p', 'of_velo', 'player_region', 'number_of_missing'
        ]
        ...
        for col in feature_columns:
            if col not in df.columns:
                if col in ['height', 'weight', 'hand_speed_max', 'bat_speed_max', 
                          ...
                          'sweet_spot_p', 'of_velo', 'number_of_missing']:
                    # Numerical features
                    default_values = {
                        ...
                        'distance_max': 300.0, 'sweet_spot_p': 0.7, 'of_velo': 75.0,
                        'number_of_missing': 0
                    }
        ...
        for col in categorical_cols:
            if df[col].isnull().sum() > 0:
                ...
                default_values = {
                    ...
                    'primary_position': 'OF'  # Default to outfielder
                }
    ...
    def predict(self, input_data: Dict) -> Dict:
        """
        Run the complete outfielder prediction pipeline.
    ...
    def get_feature_info(self) -> Dict:
        ...
        feature_info = {
            ...
            'descriptions': {
                ...
                'primary_position': 'Primary position (OF, CF, LF, RF)'
            }
        }
